"""Write compact ATR wakeup + position_freeze tester grid configs."""
...
def _render_config(
    ...
    *,
    ...
    return f"""# Auto-generated Mode D tester ATR + position_freeze grid config #{idx}
# params: trade_mode={trade_mode}, time_filter=true, atr_expansion.short_window=30, atr_expansion.long_window=400, atr_expansion.min_ratio={ratio}, ttl.bars={ttl_bars}, no_fresh_candidate.enabled=false, position_freeze.enabled=true, position_freeze.min_hold_bars={min_hold_bars}
        ...
        for min_ratio in MIN_RATIOS:
            for ttl_bars in TTL_BARS:
                for min_hold_bars in MIN_HOLD_BARS:
                    idx += 1
                    filename = (
                        f"modeD_atr_freeze_{idx:03d}_{trade_mode}"
                        f"_sw030_lw0400_mr{_ratio_tag(min_ratio)}"
                        f"_ttl{ttl_bars:03d}_hold{min_hold_bars:02d}.yaml"
                    )
