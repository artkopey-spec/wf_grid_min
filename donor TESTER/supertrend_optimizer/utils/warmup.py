...
def calculate_warmup(n: int, cfg: Dict[str, Any]) -> int:
    ...
    atr_period_max = atr_range[1] if atr_range else DEFAULT_ATR_PERIOD_MAX
...
def calculate_warmup_tester(n: int, atr_period: int, warmup_period_auto: bool = False) -> int:
    """
    Calculate warmup period for tester (simplified version without full config).
    
    For tester, we use a simpler formula:
        if warmup_period_auto:
            auto = clamp(int(n * 0.10), 100, 400)
            warmup = max(auto, atr_period)
        else:
            warmup = atr_period (or provided warmup_period)
    
    Args:
        n: Number of data points
        atr_period: ATR period (used as minimum warmup)
        warmup_period_auto: Whether to use auto-warmup calculation
